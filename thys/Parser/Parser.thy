...
section \<open>Parsing tokens to Concrete Syntax Trees\<close>
  
code_printing
  code_module Syntax_Parser \<rightharpoonup> (Haskell) \<open>
...
import ExternalLibrary_Located (Located (..), getPos, unLoc)
import qualified Syntax_CST as CST
import Syntax_Errors
import Syntax_Tokens (Token (..), showToken)
...
axiomatization
  run_parser :: \<open>String.literal \<Rightarrow> token located list \<Rightarrow>
    (String.literal diagnostic + (CST.module located \<times> String.literal diagnostic))\<close>
...
code_printing
  constant run_parser \<rightharpoonup> (Haskell) "Syntax'_Parser.runParser"
