...
section \<open>Lexing a string to a list of tokens\<close>

text \<open>
  We first define the lexer as a pure Haskell implementation.
  Since we do not plan on targetting any other language (mainly because we will rely on the
  \<^bold>\<open>diagnose\<close> library), this is not a problem.
  If we want to target other languages (why?) then we'll have a lot more work to do (for example,
  providing a uniform interface for error diagnostics).
\<close>

code_printing code_module Syntax_Lexer \<rightharpoonup> (Haskell) \<open>
...
import ExternalLibrary_Located (Located (..), Position (..))
import Syntax_Errors
import Syntax_Tokens (Token (..))
...
text \<open>
  Next we define Isabelle bindings for our Haskell code.
\<close> 

axiomatization
  run_lexer :: \<open>String.literal \<Rightarrow> String.literal \<Rightarrow>
    (String.literal diagnostic + (token located list \<times> String.literal diagnostic))\<close>

code_reserved Haskell runLexer
code_printing
  constant run_lexer \<rightharpoonup> (Haskell) "Syntax'_Lexer.runLexer'"
